...
@when('The string "{value}" posted to the "{endpoint_name}" endpoint using PUT')
def step_impl(context, value, endpoint_name):
    try:
        response = test_client.put(url=f"/{endpoint_name}", params={"filename": value.strip()})
        if response.status_code == 200:
            context.response = response
            context.success = True
        else:
