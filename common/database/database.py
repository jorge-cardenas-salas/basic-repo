...
def mssql_url():
    """
    Create a URL for SQL Server. If a more robust implementation is needed
    IMPORTANT: Un-comment MS-SQL specific installs in Dockerfile if SQL Server is to be used

    Returns:
        URL: The URL for SQL Server
    """
    connection_string = (
        r"Driver={ODBC Driver 18 for SQL Server};"
        f"Server={DB_CONN['server']},{DB_CONN['port']};"
        f"Uid={DB_CONN['user']};"
        f"Pwd={DB_CONN['pwd']};"
        f"Database={DB_CONN['name']};"
        r"Encrypt=yes;"
...
# Get DB engine
engine = create_engine(sqlite_str() if USE_SQLITE else mssql_url())
